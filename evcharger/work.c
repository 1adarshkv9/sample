...
{
    int ret = 0;
    pthread_attr_t attr_db_listener, attr_CAN_listener;
    struct sched_param param_db_listener, param_CAN_listener;

    pthread_t db_listener;
    pthread_t CAN_listener;

    // Initialize thread attributes
    pthread_attr_init(&attr_db_listener);
    pthread_attr_init(&attr_CAN_listener);

    // Set thread priorities
    param_db_listener.sched_priority = sched_get_priority_max(SCHED_FIFO);
    pthread_attr_setschedparam(&attr_db_listener, &param_db_listener);

    param_CAN_listener.sched_priority = sched_get_priority_max(SCHED_FIFO) - 1;
    pthread_attr_setschedparam(&attr_CAN_listener, &param_CAN_listener);
    ...
    ret = pthread_create(&db_listener, &attr_db_listener, &dbus_listener, NULL);
    if(ret != 0)
    {
    ...
    // Thread for CAN read
    ret = pthread_create(&CAN_listener, &attr_CAN_listener, &CAN_read_thread, NULL);
    if(ret != 0)
    {
    ...
    pthread_attr_destroy(&attr_db_listener);
    pthread_attr_destroy(&attr_CAN_listener);
